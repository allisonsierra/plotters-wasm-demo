...
impl Chart {
    ...
    /// Return `Chart` struct suitable for coordinate conversion.
    pub fn mandelbrot(canvas: HtmlCanvasElement, x_min: f64, x_max: f64, y_min: f64, y_max: f64) -> Result<Chart, JsValue> {
        let map_coord = mandelbrot::draw(canvas, x_min, x_max, y_min, y_max).map_err(|err| err.to_string())?;
        Ok(Chart {
